...
function Footer() {
  return (
    <div className="mt-48 bg-[#141414] border-t border-white/5 px-8 py-12 text-white/48">
      <div className="max-w-5xl mx-auto">
        ...
        <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-x-4 gap-y-3 mb-8">
          {links.flat().map((label) => (
            <li key={label}>
              <Link
                href="#"
                className="text-sm hover:text-white/60 transition-colors underline"
              >
                {label}
              </Link>
            </li>
        ...
        <div className="flex items-center gap-3 mb-6">
          <button className="flex items-center gap-2 text-xs border border-white/25 px-3 py-1.5 hover:text-white/60 transition-colors">
            <Languages scale={16} />
            English
          </button>
        </div>
