...
function VideoControls({ title, thumbnails }: Props) {
  const timeState = usePlayer(selectTime);
  const bufferState = usePlayer(selectBuffer);

  const duration = timeState?.duration ?? 0;
  ...
  return (
    <div
      className={`fixed top-0 left-0 h-full w-full ${!controlsVisible ? "curosor-none" : ""}`}
          ...
          <div className="w-full flex items-center gap-4">
            <div className="flex-1">
              <Slider.Root
                ...
                max={duration}
                value={currentTime}
                onValueChange={(t) => timeState?.seek(t)}
                onValueCommit={(t) => timeState?.seek(t)}
              ...
              >
                <Slider.Track className="relative w-full h-1 group-hover:h-2 bg-white/50 rounded-full overflow-hidden">
                  <Slider.Buffer
              ...
              <div className="group relative flex items-center">
                ...
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 pb-2 hidden group-hover:flex">
                  <div className="px-2 py-6 bg-brand-background/90 rounded-[0.135rem]">
                    ...
                    >
                      <VolumeSlider.Track className="relative h-full w-3 bg-white/30 rounded-xs overflow-hidden">
                        <VolumeSlider.Fill
