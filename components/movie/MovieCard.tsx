...
function MovieCard({ movie }: Props) {
  ...
  return (
    <motion.div
      className={`u-movie-card-shadow relative group cursor-pointer bg-[#181818] rounded-sm ${isElevated ? "z-10" : "z-0"}`}
      style={{
        transformOrigin,
      }}
      ...
      ref={ref}
    >
      <Link href={`/watch/${movie.publicId}`}>
        <Image
          src={movie?.thumbnailUrl ?? ""}
          alt={movie.title}
          width={300}
          height={150}
          className={`w-full h-auto object-cover rounded-sm transition-opacity duration-200 ${isPlaying ? "opacity-0" : "opacity-100"}`}
        />
        <video
          ref={videoRef}
          ...
          <div className="p-4 flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1">
                <button className="w-8 h-8 flex items-center rounded-full bg-white hover:bg-white/80 text-black">
                  <PlayButtonIcon size={20} className="ml-2" />
                </button>
                <Tooltip>
